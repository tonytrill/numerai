# -*- coding: utf-8 -*-
"""
NN with Selecting only those elements that are "close" to the test set
"""
...
#extra_eras = pd.DataFrame(submit.loc[(submit.data_type == "validation") &  
#                         (submit.era == "era99")  | (submit.era == "era98")
#                        | (submit.era == "era100") | (submit.era == "era101")
#                        | (submit.era == "era103") | (submit.era == "era106")
#                        | (submit.era == "era102"), :])
extra_eras = pd.DataFrame(submit.loc[(submit.data_type == "validation") &  
                         (submit.era == "era102")  | (submit.era == "era103")
                        | (submit.era == "era104") | (submit.era == "era105")
                        | (submit.era == "era106") | (submit.era == "era107")
                        | (submit.era == "era103"), :])
extra_eras = extra_eras.append(submit.loc[(submit.data_type == "validation") &  
                         (submit.era == "era102")  | (submit.era == "era103") 
                         | (submit.era == "era106") | (submit.era == "era107"), :])

#extra_eras = pd.DataFrame(submit.loc[submit.data_type=="validation",:])
extra_eras = create_features(extra_eras)
...
x_train = x_train.append(x_extra_era)
y_train = y_train.append(y_extra_era)
...
# Performing really bad on eras 102,103,106,107.
# Added poor performing eras to the model in order to have "Consistency" metric.
# I didn't train the model with the entire validation set just a few eras
for i in list(submit.loc[submit.data_type=="validation",:].era.unique()):
    prob = submit.loc[submit.era==i,"probability"]
    true = submit.loc[submit.era==i,"target"]
    print(i, log_loss(true,prob))